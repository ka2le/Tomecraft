...
export function loadLibrary(): Library & { warning?: string } {
  const fallback = { spells: starterSpells, slots: starterSpells.slice(0, 4).map(s => s.id) };
  try {
    ...
    const slots = Array.from({length: 4}, (_, i) => typeof value.slots?.[i] === 'string' && ids.has(value.slots[i]) ? value.slots[i] : spells[i % spells.length].id);
    return { spells, slots };
...
export function saveLibrary(library: Library): void {
  localStorage.setItem(STORAGE_KEY, JSON.stringify({ version: 1, ...library }));
}
