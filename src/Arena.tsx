...
export default function Arena({ spells, slots, onSlots, activeId, onActive, visible, notify }: { spells: Spell[]; slots: string[]; onSlots: (slots: string[]) => void; activeId: string; onActive: (id: string) => void; visible: boolean; notify: (text: string) => void }) {
  ...
  const boundCamera = () => { camera.current.x=Math.min(ROOM.width+150,Math.max(-150,camera.current.x));camera.current.y=Math.min(ROOM.height+100,Math.max(-100,camera.current.y)); };
  ...
  const castAt = (point:Point) => { if(game.current?.cast(activeRef.current,point)) setCastCount(game.current.casts); else if(point.x<15||point.x>1385||point.y<15||point.y>985) notifyRef.current('Aim inside the chamber walls.'); };
